...
export class NacionalnostDialogComponent implements OnInit {
  ...
  public addNacionalnost(): void {
    this.nacionalnostService.addNacionalnost(this.data).subscribe(() => {
      this.snackBar.open(
        'Uspešno dodata nacionalnost: ' + this.data.naziv,
        'OK',
        {
          duration: 2500,
        }
      ...
      (error: Error) => {
        ...
        this.snackBar.open(
          'Došlo je do greške prilikom dodavanja nove nacionalnosti! ',
          'Zatvori',
          {
            duration: 2500,
          }
  ...
  public updateNacionalnost(): void {
    this.nacionalnostService.updateNacionalnost(this.data).subscribe(() => {
      this.snackBar.open(
        'Uspešno modifikovana nacionalnost: ' + this.data.naziv,
        'OK',
        {
          duration: 2500,
        }
      ...
      (error: Error) => {
        console.log(error.name + ' ' + error.message);
        this.snackBar.open(
          'Došlo je do greške prilikom modifikacije postojeće nacionalnosti! ',
          'Zatvori',
          {
            duration: 2500,
          }
  ...
  public deleteNacionalnost(): void {
    this.nacionalnostService.deleteNacionalnost(this.data.id).subscribe(() => {
      this.snackBar.open(
        'Uspešno obrisana nacionalnost: ' + this.data.naziv,
        'OK',
        {
          duration: 2500,
        }
      ...
      (error: Error) => {
        console.log(error.name + ' ' + error.message);
        this.snackBar.open(
          'Došlo je do greške prilikom brisanja nacionalnosti! ',
          'Zatvori',
          {
            duration: 2500,
          }
